...
def generate_launch_description():
    ...
    default_model_path = os.path.join(urdf_package_path, 'urdf', 'first_robot.urdf')    # /* 获取urdf文件路径 */
    ...
    rviz_node = launch_ros.actions.Node(
        package = 'rviz2',
        executable = 'rviz2',
        arguments = ['-d', default_rviz_config_path]    # 直接使用保存配置好的rviz文件，arguments相当于直接在后面加入 -d+路径
    )
